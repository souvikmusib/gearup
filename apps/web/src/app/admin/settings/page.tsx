    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: 'Quick Line Items', href: '/admin/settings/quick-items', desc: 'Pre-configured items for one-tap add on invoices' },
        ].map((item) => (
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
        ...
        <button onClick={async () => { const res = await fetch('/api/admin/settings/export', { credentials: 'same-origin' }); if (res.status === 401) { alert('Not authenticated. Please login again.'); return; } if (res.ok) { const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `gearup-backup-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url); } else { alert('Export failed'); } }} className="inline-flex rounded-lg bg-gray-700 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">⬇ Export Backup</button>
      </div>
