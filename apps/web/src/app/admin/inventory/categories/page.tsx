...
export default function InventoryCategoriesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await api.post('/admin/inventory/categories', form);
    setSaving(false);
  ...
  const saveEdit = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true);
    const res = await api.patch(`/admin/inventory/categories/${editItem.id}`, editForm);
    setSaving(false);
  ...
  const remove = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm('Delete this category? Items using it will be affected.')) return;
    const res = await api.delete<any>(`/admin/inventory/categories/${id}`);
    if (res.success) load();
  };
