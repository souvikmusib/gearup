  ...
  return (
    <Modal open={!!itemId} onClose={onClose} title={`Edit: ${sku}`}>
      <form onSubmit={save} className="space-y-3">
        ...
        <div><label className={labelCls}>Company / Brand (manufacturer)</label><input className={inputCls} placeholder="e.g. NGK, Motul, Hero" value={form.brand} onChange={e => setForm({ ...form, brand: e.target.value })} /></div>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className={labelCls}>MRP</label><input type="number" step="0.01" className={inputCls} value={form.mrp} onChange={e => { const mrp = e.target.value; const dp = Number(form.discountPercent) || 0; const sp = mrp ? String((Number(mrp) * (1 - dp / 100)).toFixed(2)) : form.sellingPrice; setForm({ ...form, mrp, sellingPrice: sp }); }} /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
