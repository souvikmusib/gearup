...
Target file: `apps/web/src/app/api/admin/reports/workers/route.ts`

## Findings to fix in this file (1)

### 1. [P1] Worker report `activeAssignments` actually counts ALL assignments ever
- _id_: `reports-workers-active-mislabeled` · _category_: business-logic
- _location_: `apps/web/src/app/api/admin/reports/workers/route.ts:12`
- _evidence_:
```
_count: { select: { assignments: true, tasks: true } } ...
activeAssignments: w._count.assignments
```
- _impact_: UI label says 'active' but value is lifetime. /reports?type=workers correctly filters by unassignedAt:null + jobCard.status — divergent reality.
- _proposed fix_: Apply same where filter (unassignedAt:null, jobCard.status notIn DELIVERED/CANCELLED/CLOSED) to _count.assignments.