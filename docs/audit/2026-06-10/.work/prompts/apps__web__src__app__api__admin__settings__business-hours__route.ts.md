...
Target file: `apps/web/src/app/api/admin/settings/business-hours/route.ts`

## Findings to fix in this file (1)

### 1. [P1] Business-hours route is GET-only — no way to edit slot rules from UI
- _id_: `business-hours-read-only` · _category_: business-logic
- _location_: `apps/web/src/app/api/admin/settings/business-hours/route.ts:1-18`
- _evidence_:
```
...
```
- _impact_: Business hours can only be changed via Prisma Studio / SQL. On go-live morning when the shop changes Sunday hours, admins have no path to fix it without engineering. UI page is also display-only.
- _proposed fix_: Add PUT to replace the whole week of rules (in a transaction: deleteMany + createMany), validate dayOfWeek 0-6, openTime<closeTime as HH:MM, slotDurationMinutes 5-240, maxCapacity 1-50. Build edit UI.