...
Target file: `apps/web/src/app/api/admin/reports/revenue/route.ts`

## Findings to fix in this file (1)

### 1. [P1] Report routes accept `from`/`to` with no validation
- _id_: `reports-no-query-validation` · _category_: validation
- _location_: `apps/web/src/app/api/admin/reports/revenue/route.ts:11`
- _evidence_:
```
const from = sp.get('from'); const to = sp.get('to');
if (from && to) where.paymentDate = { gte: new Date(from), lte: new Date(to) };
```
- _impact_: Garbage input yields Invalid Date → 500. to<from silently returns empty. Repeats in /reports/expenses and /reports?type=...
- _proposed fix_: Zod schema z.object({ from: z.string().date().optional(), to: z.string().date().optional() }).refine(from<=to).